...
ado-axi pr abandon <id> [--repo <repo>]    # close without merging; already-abandoned is a no-op
...
`--status draft` keeps only the active pull requests flagged as drafts, which is how a draft PR
works as a claim other agents can see; `--status active` returns drafts alongside ready ones.
...
an already completed PR as a no-op. `pr abandon` closes one without merging and refuses a completed
PR. Run `pr checks` before completion to see concise policy/status
